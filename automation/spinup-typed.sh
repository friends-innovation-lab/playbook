...
# ── Resume mode ────────────────────────────────────────────────────────────
# When --resume is passed, skip the full spinup and just finish Supabase
# credential retrieval + env var propagation.

if $RESUME; then
    echo ""
    echo -e "${BOLD}╔══════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${BOLD}║   Friends Innovation Lab — Spinup Resume                   ║${NC}"
    echo -e "${BOLD}╠══════════════════════════════════════════════════════════════╣${NC}"
    ...
    if [[ ! -d "$LOCAL_PROJECT_PATH" ]]; then
        info "Project folder not found locally — cloning from GitHub first..."
        mkdir -p "$PROJECTS_DIR"
        git clone "https://github.com/${GITHUB_ORG}/${PROJECT_NAME}.git" "$LOCAL_PROJECT_PATH"
        ok "Cloned to ${LOCAL_PROJECT_PATH}"
    else
        ok "Local project found at ${LOCAL_PROJECT_PATH}"
    fi
    ...
    for i in {1..36}; do
        ...
        info "  Status: ${STATUS:-unknown} (poll $i/36)..."
        sleep 10
    done

    if ! $SUPABASE_READY; then
        fail "Supabase still not ready after 6 minutes (status: ${STATUS:-unknown})"
        echo "  Wait a few more minutes and run --resume again."
        exit 1
    fi
    ...
    if [[ -f "${LOCAL_PROJECT_PATH}/.env.example" && ! -f "$LOCAL_ENV_PATH" ]]; then
...
EOF
    ...
    echo ""
    echo -e "${BOLD}╔══════════════════════════════════════════════════════════════╗${NC}"
    echo -e "${BOLD}║   Resume complete                                          ║${NC}"
    echo -e "${BOLD}╠══════════════════════════════════════════════════════════════╣${NC}"
    ...
    echo "  3. npm run dev"
    echo ""
    echo -e "${GREEN}Done!${NC} Resume complete for $PROJECT_NAME."
    exit 0
fi
...
if $SKIP_SUPABASE; then
    ...
    if [[ -n "$EXISTING_REF" ]]; then
        ...
        CREATE_OUTPUT="$(supabase projects create "${PROJECT_NAME}" \
            ...
            --region us-east-1 \
            --db-password "$SUPABASE_DB_PASSWORD" 2>&1 || true)"
        ...
        if [[ -z "$SUPABASE_PROJECT_REF" ]]; then
            warn "Could not determine Supabase project ref — --resume will look it up by name"
        else
